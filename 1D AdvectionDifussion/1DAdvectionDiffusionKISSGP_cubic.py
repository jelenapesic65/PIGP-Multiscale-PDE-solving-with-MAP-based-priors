...
def cubic_u(s):
    ...
    out[mask1] = 1.5*s[mask1]**3 - 2.5*s[mask1]**2 + 1
    out[mask2] = -0.5*s[mask2]**3 + 2.5*s[mask2]**2 - 4*s[mask2] + 2
...
def cubic_u_d1(s):
    ...
    out[mask1] = sign[mask1]*(4.5*a[mask1]**2 - 5*a[mask1])
    out[mask2] = sign[mask2]*(-1.5*a[mask2]**2 + 5*a[mask2] - 4)
...
def interp_weights_d1(x, U, h):
    ...
    for k in [-1, 0, 1, 2]:
        idx = j + k
        s = (x - U[idx]) / h
        W[idx] = cubic_u_d1(np.array([s]))[0] / h
...
def cubic_u_d2(s):
    ...
    out[mask1] = 9*a[mask1] - 5
    out[mask2] = -3*a[mask2] + 5
...
def interp_weights_d2(x, U, h):
    ...
    for k in [-1, 0, 1, 2]:
        idx = j + k
        s = (x - U[idx]) / h
        W[idx] = cubic_u_d2(np.array([s]))[0] / h**2
...
def nlml(u,x,theta,with_bc=False):
    ...
    Kuu, W = build_K(u,x,c,l,sigma,sigma_n,v,kappa,with_bc=with_bc)
    if not with_bc:
        x = x[2:]  # Remove boundary points
    K = W @ Kuu @ W.T + sigma_n**2 * np.eye(len(x)+len(c))
...
x_star = np.sort(np.random.uniform(a, b, nt))
h = u[1] - u[0]
#idx_f = np.arange(0,n)
idx_u = np.arange(0,m)
#yt = f(x_star)
...
Kuu, W = build_K(u,x,c,l_opt, sigma_opt, sigma_n_opt, v_opt, kappa_opt, with_bc=True)
K = W @ Kuu @ W.T +sigma_n_opt**2 * np.eye(nxc)
print("cond(K) =", np.linalg.cond(K))
Kuu,Wstar = build_K_star(u,x,c,x_star, l_opt, sigma_opt, sigma_n_opt, v_opt, kappa_opt, with_bc=True)
...
Kuu_int, W_int = build_K(u,x,c,l_opt, sigma_opt, sigma_n_opt, v_opt, kappa_opt, with_bc=False)
K_int = W_int @ Kuu_int @ W_int.T + sigma_n_opt**2 * np.eye(nxc-2)
Kuu_int, W_star_int = build_K_star(u,x,c,x_star, l_opt, sigma_opt, sigma_n_opt, v_opt, kappa_opt, with_bc=False)
...
for xi in np.linspace(a,b,20):
    print(np.sum(interp_weights(xi, u, h)))
