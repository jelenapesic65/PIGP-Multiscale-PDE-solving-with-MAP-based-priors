...
def nlml(u,x,y,theta,with_bc=False):
    ...
    n_system = W.shape[0]
    def K_mv(v):
        return W @ (Kuu @ (W.T @ v)) + sigma_n**2 * v
...
def K_mv(v):
    return W @ (Kuu @ (W.T @ v)) + sigma_n_opt**2 * v
...
#u_mean = Wstar @ Kuu @ W.T @ alpha #here the K is fully formed and that defeates the purpose of W being sparse
...
K_int = LinearOperator((nxc-2, nxc-2), matvec=lambda v: W_int @ (Kuu_int @ (W_int.T @ v)) + sigma_n_opt**2 * v)
