...
N_OBS      = 200           # interior observations
...
def kissgp_posterior(U, x_obs, y_obs, x_col, y_col,
    ...
    alpha, info = cg(K, y_all,  maxiter=10000)
    ...
    for i in range(K_star_rows.shape[0]):
        v_i, _ = cg(K, K_star_rows[i], maxiter=10000)
        V_mat[:, i] = v_i
...
for kname in ['rbf', 'matern52']:
    ...
    std1 = np.sqrt(var1)
    res1 = numerical_pde_residual(mu1, x_star, v_opt, kap_opt)
...
plt.figure()
plt.plot(x_star,res1, label='PDE residual of KISSGP posterior')
